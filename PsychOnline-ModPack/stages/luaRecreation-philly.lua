...
function onUpdate(elapsed)
	...
	local phillyAlpha = getProperty('phillyWindow.alpha') - ((crochet / 1000) * elapsed * 1.5)
	setProperty('phillyWindow.alpha', phillyAlpha)
end
...
function updateTrainPos()
	...
	if startedMoving then
		...
		if getProperty('phillyTrain.x') < -4000 and finishing then
			trainReset()
		end
	end
end
...
function onBeatHit()
	...
	local isTrainActive = (luaSpriteExists('phillyTrain') and getProperty('phillyTrain.visible'))
