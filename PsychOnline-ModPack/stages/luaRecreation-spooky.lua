...
function onCreatePost()
	...
	if songName == 'B-Side Spookeez (Classic)' then
		lightningOffset = 16
	elseif songName == 'B-Side South (Classic)' then
		lightningOffset = 64
	end
end
...
function lightningStrikeShit()
	...
	if cameraZoomOnBeat then
		...
		if not getProperty('camZooming') then
			doTweenZoom('camGameTween', 'camGame', getProperty('defaultCamZoom'), 0.5, 'sineOut')
			doTweenZoom('camHUDTween', 'camHUD', 1, 0.5, 'sineOut')
		end
	end
